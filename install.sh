...
# I wasn't able to write a new file, I think that it's my unique option. Hope I help a bit.
...
spinner()
{
  ...
  linuxosis="linux"
          echo "Operating System detected is $OSTYPE"
          echo "Installing Required Packages..."
  sudo yum -qq -y upgrade & spinner
  sudo yum -qq -y install nodejs & spinner
  sudo yum -qq -y install npm & spinner
  sudo yum -qq -y install php5 & spinner
  elif [[ "$OSTYPE" == "darwin"* ]]; then
         echo "This operating system is unsupported!"
        echo
        echo "I recommend you install Ubuntu."
